...
public class Tree
{
	public static void main(String[] args)
	{
		...
		System.out.println("Enter length of subbranch as a "
				+ "fraction of the length of its parent: (eg .6)");
		final double lengthScaleFactor = keyboardInput.nextDouble();
		System.out.println("Enter the angle in degrees that a subbranch "
...
of the length of the branch it is coming off of.
		...
		drawFractalTree((Graphics2D) g);
	     ...
	     // Print the image to a file, if asked.
	      if (printFiles)
	      {
	         ...
	         try
	         {
	            ImageIO.write(bi, "JPEG", new File("KochSnowflake" + 
	...
	public void drawLine(Graphics2D g2, int n, Point2D p1, Point2D p2)
	{
		...
		Point2D	pB;
		g2.setColor(new Color(118-((currentLevelOfRecursion-n)*97/(maxLevelsOfRecursion)),
				64+((currentLevelOfRecursion-n)*23/(maxLevelsOfRecursion)),
				41-((currentLevelOfRecursion-n)*41/(maxLevelsOfRecursion))));
		g2.setStroke(new BasicStroke(10 -((currentLevelOfRecursion-n)*9.75f/(currentLevelOfRecursion+1)),BasicStroke.CAP_ROUND,BasicStroke.JOIN_MITER));		
